...
Trang này hướng dẫn bạn từ lúc mở Jarvis lần đầu tiên cho tới khi Jarvis sẵn sàng trò chuyện: tạo tài khoản admin, đăng nhập Claude làm "bộ não", chọn model, và kiểm tra trạng thái hệ thống ở trang Tổng quan.

## Tính năng này là gì

Jarvis OS là một lớp điều hành AI chạy trên máy hoặc VPS của bạn, lấy Claude Code làm bộ não. Trước khi dùng được, Jarvis cần 3 thứ:
...
Mở `http://localhost:7777` (hoặc địa chỉ VPS của bạn). Nếu đây là lần đầu và chưa có tài khoản, Jarvis hiện cửa sổ **Chào mừng tới Jarvis** với 3 mục đánh số sẵn.

Nếu bạn chạy trên máy cá nhân (localhost), mục mật khẩu và MÃ THIẾT LẬP là tùy chọn, có thể bỏ trống. Nếu bạn chạy công khai (VPS/Docker), Jarvis bắt buộc bạn đặt mật khẩu và nhập MÃ THIẾT LẬP mới cho qua.

### Bước 2: Đặt tên Workspace

Ở mục **1. Workspace**, gõ tên hiển thị vào ô **Tên hiển thị** (ví dụ tên cửa hàng hoặc tên bạn). Bỏ trống thì Jarvis dùng mặc định là "Jarvis OS". Đây chỉ là nhãn hiển thị, đổi lại bất cứ lúc nào.
...
3. Nếu Jarvis chạy công khai, một ô **Mã thiết lập** sẽ hiện ra. Dán MÃ THIẾT LẬP vào đây (cách lấy xem mục "Khi nào cần MÃ THIẾT LẬP" bên dưới).

Trên máy cá nhân, nếu bạn để trống mật khẩu thì Jarvis không đặt tài khoản và ai mở link máy này cũng dùng được. Chỉ nên bỏ trống khi máy chỉ mình bạn dùng.
...
| **ChatGPT (gói subscription)** | Đăng nhập ChatGPT Plus/Pro qua Codex, vẫn dùng được MCP của Jarvis. | Đăng nhập ChatGPT ở trang Models |
...
Bấm **Bắt đầu dùng Jarvis →** để lưu và vào app.
...
4. Jarvis hiện một đường link. Bấm mở link đó (mở trong tab mới) để đăng nhập tại claude.ai.
5. Nếu sau khi đăng nhập trang hiện một mã code, dán mã đó vào ô **dán code (nếu có)** rồi bấm **Gửi code**. Một số luồng không cần dán code, Jarvis tự nhận biết và cập nhật trạng thái.
...
Nút **↻ Kiểm tra lại** trên thẻ dùng để nạp lại trạng thái đăng nhập bất cứ lúc nào. Nút **Ngắt** dùng để đăng xuất Claude khỏi Jarvis.
...
Hiển thị phiên bản Jarvis đang chạy (dạng `v0.4.0`) và cho biết có bản mới trên GitHub hay không.

- Bấm **Kiểm tra lại** để so lại với bản mới nhất.
- Nếu có bản mới, nút **⬆ Cập nhật ngay** hiện ra. Bấm để tải bản mới và khởi động lại (app tự tải lại trang sau khoảng 20 đến 40 giây). Dữ liệu của bạn không mất khi cập nhật. Lưu ý: bản Docker cần bật thêm dịch vụ Watchtower mới tự cập nhật được từ nút này, nếu không Jarvis sẽ báo chạy `./update.sh` thủ công.
...
- Nếu màn hình hẹp (điện thoại), Jarvis tự ép chế độ nhẹ (lite-mode) dù công tắc đang bật.
...
Thao tác này an toàn: chỉ di chuyển khi thư mục đích chưa có, không ghi đè, chạy lại nhiều lần cũng vô hại (ví dụ chuyển `Jarvis/agents` sang `agents`, `Memory` sang `memory`; có git backup). Sau khi chạy, Jarvis báo đã di chuyển gì hoặc "Không có gì cần di chuyển (đã chuẩn)".
...
**MÃ THIẾT LẬP (setup token)** chỉ xuất hiện khi Jarvis chạy công khai (nghe trên `0.0.0.0`, tức VPS/Docker/Hostinger) và chưa có tài khoản admin. Vì lúc này Claude chạy với toàn quyền trên máy, Jarvis không cho phép bất kỳ ai chỉ có đường link cũng tạo được tài khoản admin. MÃ THIẾT LẬP là chuỗi bí mật chỉ in ra log/terminal của server, nên chỉ người có quyền xem server mới lấy được.

Trên máy cá nhân (localhost), Jarvis không hỏi mã này.
...
Sau khi có mã, dán vào ô **Mã thiết lập** trong wizard rồi bấm **Bắt đầu dùng Jarvis →**. Mã được dùng một lần, sau khi tạo tài khoản thành công Jarvis xóa mã đi.

**Cách khỏi cần mã:** khi deploy, đặt sẵn hai biến môi trường `JARVIS_ADMIN_USER` và `JARVIS_ADMIN_PASSWORD`. Jarvis tự tạo tài khoản admin lúc khởi động, mở app ra là màn đăng nhập luôn, không hỏi MÃ THIẾT LẬP. Chi tiết biến môi trường xem [Cấu hình .env](16-cau-hinh-env.md).
...
- **Sai quá nhiều lần khi đăng nhập, bị báo "Quá nhiều lần sai":** Jarvis khóa tạm để chống dò mật khẩu. Đợi ít phút rồi thử lại.
