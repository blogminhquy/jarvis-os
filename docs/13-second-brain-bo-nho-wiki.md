...
Second Brain là "bộ não ngoài" của Jarvis: một thư mục chứa các ghi chú Markdown mà Jarvis đọc, tích luỹ và nhớ lâu dài. Nhờ nó, Jarvis không chỉ trả lời câu hỏi trong lúc chat mà còn nhớ về anh, về việc kinh doanh của anh, và ngày càng hiểu anh hơn theo thời gian.

Trang này hướng dẫn: hiểu Second Brain gồm những gì, cách tạo và chọn nhiều "não" khác nhau, cách để Jarvis nhớ (bộ nhớ dài hạn), và cách "tiêu hoá" tài liệu (INGEST) để biến file thô thành tri thức dùng lại được.
...
| Memory | `memory/` | Bộ nhớ sống: những gì Jarvis nhớ về anh và doanh nghiệp. |
...
- **Wiki** là tri thức đã tinh lọc, nối với nhau thành mạng, chính là thứ Jarvis vẽ ra trong [Đồ thị tri thức 3D](03-do-thi-tri-thuc-3d.md).
- **Memory** là bộ nhớ dài hạn giúp Jarvis "nhớ anh".
...
## Đa-brain: nhiều bộ não trong một Jarvis
...
3. Toàn bộ Jarvis (chat, đồ thị, bộ nhớ, agents) lập tức chuyển sang brain đó. Không cần tải lại trang.

### Tạo brain mới

1. Bấm nút ➕ ở cạnh ô chọn brain.
2. Nhập **Tên brain mới** vào ô hiện ra, rồi xác nhận.
3. Jarvis tạo một thư mục con mới kèm sẵn cấu trúc chuẩn (sources, wiki, memory, agents, workflows, attachments) và chọn ngay brain vừa tạo.
...
3. Jarvis dùng thư mục đó làm brain. Lưu ý: folder ngoài chỉ được "trỏ tới", nút 🗑 sẽ không xoá nó khỏi ổ đĩa (chỉ bỏ khỏi danh sách).
...
4. Nếu gõ đúng, brain bị xoá và Jarvis quay về Brain Default. Gõ sai tên thì thao tác bị huỷ.
...
## Bộ nhớ dài hạn: làm Jarvis "nhớ anh"

Bộ nhớ sống nằm ở `memory/` trong brain đang chọn, gồm:

- `memory/MEMORY.md`: chỉ mục, mỗi ký ức một dòng. File này được nạp sẵn vào đầu mỗi câu hỏi, nên Jarvis luôn "nhớ nền" về anh.
...
Jarvis phân 4 loại ký ức: thông tin về anh (`user`), cách anh thích làm việc (`preference`), sự thật về kinh doanh (`business`), và quyết định đã chốt (`decision`).
...
Mô tả ngay dưới ghi rõ: "Ký ức lưu ngay trong vault đang chọn - đổi máy chỉ cần trỏ lại vault là Jarvis nhớ như cũ." Nghĩa là bộ nhớ đi theo thư mục, không kẹt trong máy nào.

### Ép Jarvis ghi nhớ một điều
...
Khi anh dùng các cụm đó, Jarvis bắt buộc tạo ngay một ký ức mới: viết một file trong `memory/facts/` và thêm một dòng vào `MEMORY.md`. Ví dụ: "Nhớ điều này: shop tôi nghỉ bán Chủ Nhật" sẽ được lưu thành một sự thật `business`.

Jarvis chỉ ghi những điều bền vững, đáng nhớ. Nó bỏ qua chuyện nhất thời và không nhân bản ký ức đã có (trùng thì cập nhật file cũ).

### Học từ hội thoại (vòng hợp nhất / rewire)

Đây là vòng lặp giúp Jarvis thông minh dần: đọc lại hội thoại gần đây, rút ra sự thật mới, gộp trùng lặp, bỏ ký ức đã sai, và **đúc kết khái niệm tái dùng được vào Wiki**.
...
2. Nút đổi thành "🧠 Đang học..." trong lúc Jarvis đọc lại hội thoại.
...
Ngay dưới đó có ô tích **Tự học sau mỗi 6 lượt**. Khi bật (mặc định), cứ sau 6 lượt hội thoại Jarvis tự chạy một vòng học nền, không làm gián đoạn anh. Bỏ tích nếu muốn tắt.

Điểm quan trọng của vòng này: Jarvis phân biệt rõ hai thứ. Sự thật về anh và doanh nghiệp vào `memory/facts/`. Còn khái niệm, framework, quy trình dùng lại được thì chưng cất thành trang Wiki (có `[[wikilink]]`). Cái nào ra cái nấy, nhờ vậy đồ thị tri thức dày lên chứ không lẫn lộn với ghi chú cá nhân.

## INGEST: tiêu hoá tài liệu thành tri thức

INGEST là quy trình biến một file thô (bài viết, ảnh chụp, ghi chú) thành nguồn trong `sources/`, rồi từ đó chưng cất lên Wiki. Kết quả: Jarvis tóm tắt, rút insight, viết Wiki và có thể gợi ý task.
...
3. Mặc định, Jarvis **chỉ đọc file rồi trả lời**, chưa lưu đi đâu. Nếu anh chỉ cần tóm tắt nhanh thì gõ câu hỏi bình thường.
4. Muốn Jarvis lưu và tiêu hoá, hãy nói rõ trong tin nhắn một trong các cụm: **"lưu vào source"**, **"ingest"**, hoặc **"ghi vào second brain"**.
5. Khi đó Jarvis sẽ:
...
6. Từ nguồn đó, Jarvis rút insight và cập nhật Wiki, đồng thời có thể đề xuất task nếu tài liệu mở ra việc cần làm.

### Ép Jarvis xử lý mẻ nguồn theo lịch

Nếu anh dồn nhiều nguồn chưa xử lý, có thể để Jarvis tự làm nền:
...
LINT tìm 8 loại vấn đề: mâu thuẫn giữa các trang, thông tin cũ chưa cập nhật (stale claim), trang không có ai trỏ tới (orphan page), khái niệm thiếu trang riêng (missing page), liên kết `[[...]]` gãy (broken wikilink), trang trùng lặp, vùng kiến thức mỏng (gap), và câu hỏi mở chưa được lấp (open-question). Đọc danh sách rồi tự quyết định sửa cái nào, đừng để Jarvis sửa hàng loạt cùng lúc.

## Mẹo

- **Tách brain theo mục đích.** Một brain cho kinh doanh, một brain cho cá nhân sẽ giúp đồ thị tri thức và bộ nhớ gọn, đỡ nhiễu.
- **Nói "nhớ điều này" cho những gì bền vững.** Ví dụ ngách sản phẩm, kênh bán chính, quyết định giá. Đừng ghi chuyện nhất thời (hôm nay bận, tin nhắn vừa gửi), Jarvis vốn đã bỏ qua loại này.
- **Muốn tiêu hoá tài liệu thì phải nói rõ.** Chỉ thả file không đủ để lưu, phải kèm cụm "lưu vào source" hoặc "ingest". Nếu chỉ hỏi bình thường, Jarvis đọc xong là thôi.
- **Bộ nhớ đi theo thư mục.** Đổi máy hoặc chuyển VPS, chỉ cần trỏ Jarvis về đúng thư mục brain là mọi ký ức và Wiki còn nguyên.
...
- **Thả file mà không thấy vào Sources.** Đúng như thiết kế: mặc định chỉ đọc. Phải gõ kèm "lưu vào source" / "ingest" trong tin nhắn thì Jarvis mới tạo file `.md` trong `sources/`.
...
- **Brain báo cấu trúc chưa chuẩn.** Vào Tổng quan, mục Cấu trúc brain, bấm **Chuẩn hóa brain đang chọn** để Jarvis gom lại các thư mục.
