...
Trang này gom các trục trặc hay gặp khi dùng Jarvis OS và cách xử lý từng bước. Phần lớn sự cố chỉ cần một trong hai thao tác: khởi động lại server, hoặc tải lại trình duyệt bằng Ctrl+Shift+R. Cuối trang có mục Câu hỏi thường gặp (FAQ) ngắn gọn.

Nếu bạn mới cài Jarvis lần đầu, xem trước [Bắt đầu & thiết lập lần đầu](01-bat-dau-thiet-lap.md). Nếu bạn đang chỉnh biến môi trường, xem [Cấu hình .env](16-cau-hinh-env.md).
...
| **Claude báo chưa đăng nhập** (Jarvis không trả lời được) | Đăng nhập lại "bộ não" Claude 1 lần. Cách trong app: mở **Models**, ở thẻ Claude Code bấm **Đăng nhập Claude**, mở link, dán code nếu được yêu cầu. Cách bằng lệnh: `claude auth login --claudeai` (Docker: chạy trong App terminal). |
...
Jarvis gồm hai phần chạy khác nhau, nên cách làm mới cũng khác:
...
Cổng mặc định của Jarvis là **7777**. Khi một tiến trình cũ chưa tắt hẳn mà bạn bật bản mới, bản mới sẽ báo lỗi vì cổng đang bận. Xử lý theo thứ tự:
...
Khi Jarvis chạy public (Docker/VPS/Hostinger), lần đầu mở app sẽ ra màn tạo tài khoản admin và có thể hỏi **MÃ THIẾT LẬP**. Đây là cơ chế chống người lạ chỉ có URL cũng tạo được tài khoản (vì Claude chạy toàn quyền trên máy). Lấy mã như sau:
...
"Bộ não" của Jarvis là Claude Code CLI, đăng nhập 1 lần và giữ qua mọi restart/update. Nếu Jarvis không trả lời hoặc báo chưa đăng nhập:
...
1. **Nhật ký hoạt động của Jarvis khi tự chạy nền**: mở **Tự cải thiện** ở thanh nav trái, kéo xuống mục **Nhật ký gần đây**. Đây là nơi ghi lại các lượt Jarvis tự thức làm nhiệm vụ. Xem [Tự cải thiện](08-tu-cai-thien.md).
...
Được. Jarvis hỗ trợ nhiều brain trong cùng một thư mục. Ở dropdown chọn brain trên giao diện, bạn có thể:
...
### Đổi giọng nói của Jarvis thế nào?
